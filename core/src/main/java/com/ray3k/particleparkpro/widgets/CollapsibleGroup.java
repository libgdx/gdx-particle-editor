...
public class CollapsibleGroup extends WidgetGroup {
    ...
    public float getMinWidth() {
        var actor = horizontal ? getSmallestMinWidth() : getLargestMinWidth();
    ...
    public float getMinHeight() {
        var actor = horizontal ? getLargestMinHeight() : getSmallestMinHeight();
        if (actor == null) return 0;
    ...
    public float getPrefWidth() {
        var actor = horizontal ? getLargestPrefWidth() : getLargestPrefHeight();
        if (actor == null) return 0;
    ...
    private Actor getSmallestMinWidth() {
        ...
        var smallestWidth = Float.MAX_VALUE;
        var smallest = getChildren().first();
        for (var actor : getChildren()) {
            var width = actor instanceof Layout ? ((Layout) actor).getMinWidth() : actor.getWidth();
    ...
    private Actor getSmallestMinHeight() {
        ...
        var smallestHeight = Float.MAX_VALUE;
        var smallest = getChildren().first();
        for (var actor : getChildren()) {
            var height = actor instanceof Layout ? ((Layout) actor).getMinHeight() : actor.getHeight();
    ...
    private Actor getLargestMinWidth() {
        ...
        var largestWidth = -Float.MAX_VALUE;
        var largest = getChildren().first();
        for (var actor : getChildren()) {
            var width = actor instanceof Layout ? ((Layout) actor).getMinWidth() : actor.getWidth();
    ...
    private Actor getLargestMinHeight() {
        ...
        var largestHeight = -Float.MAX_VALUE;
        var largest = getChildren().first();
        for (var actor : getChildren()) {
            var height = actor instanceof Layout ? ((Layout) actor).getMinHeight() : actor.getHeight();
    ...
    private Actor getSmallestPrefWidth() {
        ...
        var smallest = getChildren().first();
        for (var actor : getChildren()) {
            var width = actor instanceof Layout ? ((Layout) actor).getPrefWidth() : actor.getWidth();
            if (width < smallestWidth) {
                smallestWidth = width;
                smallest = actor;
            }
    ...
    private Actor getSmallestPrefHeight() {
        ...
        var smallest = getChildren().first();
        for (var actor : getChildren()) {
            var height = actor instanceof Layout ? ((Layout) actor).getPrefHeight() : actor.getHeight();
            if (height < smallestHeight) {
                smallestHeight = height;
                smallest = actor;
            }
